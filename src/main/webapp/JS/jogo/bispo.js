 ...
 */
Bispo = function (_cor, _linha, _coluna){
    ...
    if (cor === 0) {
        ...
        imagem.src = "JS/jogo/pecas/bispo-branco.png";
    } else {
        id = id + "Preto-" + _linha + "" + _coluna;
        imagem.src = "JS/jogo/pecas/bispo-preto.png";
    }
    ...
    Bispo.inicioMovimento = function (event, ui) {
        ...
        var movimentos = Bispo.movimento(linha, coluna, corPeca);

        for (var i = 0; i < movimentos.length; i++) {
            $("#" + movimentos[i]).droppable({scope: id, drop: Bispo.jogada});
        }
    };

    Bispo.jogada = function (event, ui) {
